...
int* GA::variateDNA(int* childDNA)
{
    if (variate_flag == 'f') //启用位翻转变异
    {
        ...
        {
            int variate_position = rand() % (dna_size * 2);
...
double* GA::getFitnessVector(int** population_matrix)
{
    ...
    y_vector = population_vector.second;

    for (int i = 0; i < population_size; i++)
        //fitness_vector[i] = fitness_function(population_x_vector[i], population_y_vector[i]);
        fitness_vector[i] = (*pf)(x_vector[i], y_vector[i]);
...
{
    fitness_vector = getFitnessVector(population_matrix);
    int maxPosition = max_element(fitness_vector, fitness_vector + population_size) - fitness_vector; //最大适应度索引
    pair<double*, double*> population_vector = decodeDNA(population_matrix);
    x_vector = population_vector.first;
    y_vector = population_vector.second;
    double maxValue = (*pf)(x_vector[maxPosition], y_vector[maxPosition]);

    for (int j = 0; j < dna_size * 2; j++)
        bestDNA[j] = population_matrix[maxPosition][j];
...
void GA::addSuperDNA(int** population_matrix)
{
    ...
    for (int j = 0; j < dna_size * 2; j++)
        population_matrix[index_random][j] = bestDNA[j];
}
...
int main()
{
    ...
    // fitness function 1 //小数点后四位精度
    //GA ga(30, 200, 300, 0.7, 0.15, -10, 10, -10, 10, 'f');//位翻转
    //GA ga(30, 200, 300, 0.8, 0.1, -10, 10, -10, 10, 'd');

    //fitness function 2 //小数点后六位精度
    //GA ga(30, 200, 300, 0.6, 0.2, 0, 10, 0, 10, 'f'); //位反转
    GA ga(30, 200, 300, 0.6, 0.09, 0, 10, 0, 10, 'd'); //柯西分布

    ga.getFunction(point_of_function);
